...
async function handleRequest(request: NextRequest) {
	...
	try {
		...
		console.error("Whoop sync route is missing authorization secrets", error);
		return new Response("Sync route is not configured", { status: 500 });
	}
	...
	if (!isWhoopConfigured()) {
		return new Response("WHOOP integration is not configured", { status: 503 });
	}
